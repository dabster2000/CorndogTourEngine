package dk.alumananx.corndogtour.engine.fysics;

import java.lang.reflect.InvocationTargetException;

import org.apache.commons.beanutils.BeanUtils;

import dk.alumananx.corndogtour.engine.model.Bike;
import dk.alumananx.corndogtour.engine.model.Biker;
import dk.alumananx.corndogtour.engine.model.TrackSegment;


public class PhysicsEngine {

	private static double trans = 95 * 0.01;
	
	public static double getPowerPerGrade(double grade) {
		return grade * 14.1 + 315;
	}
	
	public static double getSpeedPerGradePerLevel(double grade, double level) {
		return  25 + (2 * level) + (Math.random() * 5.0) - (grade * 3.5);
	}
	
	public static double getActualPowerUsed(Biker biker, Bike bike, TrackSegment ts, double speed, int groupSize) {
		...
		try {
			...
			if(groupSize > 8) groupSize = 8;
			groupSize -= ts.getGrade() / 2;
			if(groupSize < 0) groupSize = 0;
			tsAdjust.setHeadwind(ts.getHeadwind() * (1.0 - ( (1.0 / 8.0) * groupSize) ) );
			
			double power = calcPower(biker, bike, tsAdjust, speed);
			return power * (1 - (0.05 * groupSize));
	...
	public static double calcVelocity(Biker biker, Bike bike, TrackSegment trackSegment, Double power) {
	...
	public static double calcTime(Biker biker, Bike bike, TrackSegment trackSegment, double power) {
	...
	public static double calcCalories(double minutes, double power) {
		return (minutes / 60.0) * (power * 4.0) / 1.163;
	}
	
	public static double calcPower(Biker biker, Bike bike, TrackSegment trackSegment, double speed) {
	...
	private static double newton(double aero, double hw, double tr, double tran,
